...
import {
  GetPayerPSPsSCT01T,
  getPayerPSPsSCT01DefaultDecoder
} from "../generated/definitions/mybank/requestTypes";
...
export type ApiOperation = TypeofApiCall<GetPayerPSPsSCT01T>;

export type ParamKeys = keyof TypeofApiParams<GetPayerPSPsSCT01T>;
...
export type WithDefaultsT<
  K extends ParamKeys | __UNDEFINED_KEY = __UNDEFINED_KEY
  > = OmitApiCallParams<GetPayerPSPsSCT01T, K>;
...
export type Client<
  ...
  > = K extends __UNDEFINED_KEY
  ? {
    readonly getPayerPSPsSCT01: TypeofApiCall<GetPayerPSPsSCT01T>;
  }
  : {
    readonly getPayerPSPsSCT01: TypeofApiCall<
      ReplaceRequestParams<
        GetPayerPSPsSCT01T,
        Omit<RequestParams<GetPayerPSPsSCT01T>, K>
      >
    >;
  };
...
export function createClient<K extends ParamKeys>({
  ...
  const getPayerPSPsSCT01T: ReplaceRequestParams<
    GetPayerPSPsSCT01T,
    RequestParams<GetPayerPSPsSCT01T>
  > = {
    method: "post",

    headers: ({
      ["X-Signature-Type"]: XSignatureType,
      ["X-Signature"]: XSignature,
      ["X-Thumbprint"]: XThumbprint
    }: {
    ...
    response_decoder: getPayerPSPsSCT01DefaultDecoder(),
    url: ({ }) => `${basePath}/getPayerPSPsSCT01`,

    body: ({ ["input"]: input }) => input as string,

    query: () => withoutUndefinedValues({})
  };
  const getPayerPSPsSCT01: TypeofApiCall<GetPayerPSPsSCT01T> = createFetchRequestForApi(
    getPayerPSPsSCT01T,
    options
  );

  return {
    getPayerPSPsSCT01: (withDefaults || identity)(getPayerPSPsSCT01)
  };
}
